...
class Test(object):
    ...
    def read_test_data(self):
        ...
        for single in singledata:
            ...
            third_mark = single.rfind("#")
            OccStr = single[2:first_mark-1]
            VehStr = single[first_mark+2:second_mark-1]
            Action = single[second_mark+1:third_mark]
    ...
    def choose_action(self,s_sliding,s_others):
        s_sliding_list = s_sliding[ np.newaxis,:]
        s_others_list = s_others[ np.newaxis,:]
        action = self.sess.run(self.a,{self.tf_s_sliding: s_sliding_list, self.tf_s_others: s_others_list})
        print(action)
